...
#(
    ...
    parameter RISCADA_OPCODE = 7'b1111011
)
(
    input  logic                             clk,
    ...
    always_comb begin
        ...
        if(issue_valid == 1'b1 && issue_instr_opcode == RISCADA_OPCODE) begin
            issue_accept        <= 1'b1;
    ...
    always_comb begin
        issue_ready     = 1'b0;
        if(issue_valid == 1'b1 && issue_instr_opcode == RISCADA_OPCODE && register_rs_valid_i == 2'b11) begin
            issue_ready         <= 1'b1;
        end
        else begin
            issue_ready         <= 1'b0; 
    ...
    assign input_all_valid = (issue_valid == 1'b1 && issue_instr_opcode == RISCADA_OPCODE && register_rs_valid_i == 2'b11);
    ...
    always_ff @(posedge clk or negedge reset_n_i) begin
        if(reset_n_i == 1'b0) begin
            ...
            state          <= 2'd1;
            $display("State 0 cu 1");
        end

        else if(state == 2'd1 && (i_result_we == 1'b0 && i_result_mem_en == 1'b0)) begin
            instr_commit   <= 1'b0;
            instr_funct7   <= 7'd0;
            instr_funct3   <= 3'd0;
            instr_opcode   <= 7'd0;
        ...
        else if(state == 2'd1 && i_result_mem_en == 1'b1) begin
            instr_commit   <= 1'b0;
            instr_funct7   <= 7'd0;
            instr_funct3   <= 3'd0;
            instr_opcode   <= 7'd0;
            ...
            result_valid   <= 1'b1;
            result_hartid  <= issue_hartid;
            result_id      <= issue_id;
            result_data    <= i_result_mem_dat ^ i_result_mem_adr;
            result_rd      <= issue_rd;
            result_we      <= 1'b1;

            state          <= 2'd2;
            $display("State 1 cu alt1");
        end 
